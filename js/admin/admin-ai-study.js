...
function aiStudyOmnirouteEndpointsMarkup() {
  ...
  return '<section id="ai-omniroute-endpoints" class="ai-panel ai-anchor-section ai-omniroute-panel" tabindex="-1" aria-labelledby="ai-omniroute-endpoints-title">' +
    ...
    '</div>' +
    '<div class="ai-omniroute-routing-note"><strong>Do not add either <code>/v1</code> address as a proxy server.</strong><span>These are upstreams used inside the selected AI proxy. In Settings → Server Role Routing, choose the trusted HTTPS root of the full <code>youtube-turbo-proxy</code> under <strong>AI Notes, Tutor &amp; AI Chat</strong>. Transcript and Turbo traffic can remain on Render independently.</span></div>' +
