...
function renderRangeView(view) {
  ...
  list.innerHTML = datesWithContent.map(ds => {
    ...
    const topicRows = items.map(it => {
      ...
      return `<div class="day-list-task-item"><span class="day-list-task-dot" style="background:${accent};"></span>${isRev?'🔁 ':''}${escapeHtml(ch.name||'')} <span style="color:var(--muted);font-size:.7rem;">${it.part||''} · ${escapeHtml(ch.subName||'')}</span></div>`;
    }).join('');
...
/* ══════════════════════════════════════════════
   EXAMZEN PHASE 2 — PLANS, UPI PAYMENTS, PLAN BADGE
══════════════════════════════════════════════ */
/* Fallback UPI — used only if admin has not saved one in Admin Panel → Plans → Payment Settings */
const EZ_UPI_FALLBACK = { upiId: 'yourname@upi', payeeName: 'ExamZen' };
...
function ezPickPlan(pid) {
  ...
  const upiDeep = 'upi://pay?pa=' + encodeURIComponent(upi.upiId) +
    '&pn=' + encodeURIComponent(upi.payeeName || 'ExamZen') +
    '&am=' + amount + '&cu=INR' +
    '&tn=' + encodeURIComponent('ExamZen ' + _ezPickedPlan.name);
...
async function ezApplyCoupon() {
  ...
  try {
    ...
    const upiDeep = 'upi://pay?pa=' + encodeURIComponent(upi.upiId) +
      ...
      '&tn=' + encodeURIComponent('ExamZen ' + _ezPickedPlan.name + ' [' + code + ']');
    const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=' + encodeURIComponent(upiDeep);
