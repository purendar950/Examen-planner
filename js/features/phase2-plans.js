/* ══════════════════════════════════════════════
   STUDYPLANNER PHASE 2 — PLANS, UPI PAYMENTS, PLAN BADGE
══════════════════════════════════════════════ */
/* Fallback UPI — used only if admin has not saved one in Admin Panel → Plans → Payment Settings */
const SP_UPI_FALLBACK = { upiId: 'yourname@upi', payeeName: 'StudyPlanner' };
let SP_PLANS = [], SP_PROFILE = null, SP_UPI = null, SP_PENDING_PAY = null, _ezPickedPlan = null, _ezCoupon = null, _ezFinalAmount = 0;

async function LoadPlans() {
  if (!_fbReady || !db) return;
  try { const s = await db.collection('plans').get(); SP_PLANS = s.docs.map(d => ({ id: d.id, ...d.data() })); } catch(e) {}
  try { const c = await db.collection('config').doc('payment').get(); SP_UPI = c.exists ? c.data() : null; } catch(e) {}
  try {
...
async function loadProfile() {
  ...
  try {
    ...
    SP_PROFILE = (s.exists && s.data().profile) || {};
  } catch(e) { SP_PROFILE = {}; }
  try {
    ...
    SP_PENDING_PAY = pend.length ? pend[0] : null;
  } catch(e) { SP_PENDING_PAY = null; }
  RenderPlanBadge();
  // FIX 1: Re-apply ALL gates now that SP_PROFILE is loaded from Firestore.
  // Without this, gates installed at loginUser() time all ran with SP_PROFILE=null
  // (which makes IsPro() return false → Gated() false → gates OPEN for everyone).
  // Calling RefreshGates() here corrects the gating state once the real plan
  // data arrives (~400ms after login).
  try { if (typeof RefreshGates === 'function') RefreshGates(); } catch(e) {}
  try { await LoadAnnouncement(); } catch(e) {}
}

/* ── Broadcast announcement banner (admin sets config/settings.announcement) ── */
let SP_ANNOUNCE = null;
async function LoadAnnouncement() {
  if (!_fbReady || !db) return;
  try {
    const s = await db.collection('config').doc('settings').get();
    const data = s.exists ? s.data() : {};
    SP_ANNOUNCE = (data && data.announcement) || null;
  } catch(e) { SP_ANNOUNCE = null; }
  RenderAnnouncement();
}
function RenderAnnouncement() {
  ...
  bar.innerHTML = '📢 ' + escapeHtml(a.text) +
    '<button onclick="DismissAnnouncement()" title="Dismiss" style="position:absolute;top:50%;right:10px;transform:translateY(-50%);background:none;border:none;color:var(--muted);font-size:1rem;cursor:pointer;">✕</button>';
}
function DismissAnnouncement() {
  if (SP_ANNOUNCE && SP_ANNOUNCE.id) {
    appState.dismissedAnnouncement = SP_ANNOUNCE.id;
...
function RenderPlanBadge() {
  ...
  if (!b) {
    ...
    b.onclick = OpenUpgrade;
    right.insertBefore(b, right.firstChild);
  }
  const today = new Date().toISOString().slice(0, 10);
  const plan = (SP_PROFILE && SP_PROFILE.plan && SP_PROFILE.plan !== 'free') ? SP_PROFILE.plan : null;
  const expired = plan && SP_PROFILE.planExpiry && SP_PROFILE.planExpiry < today;
  if (plan && !expired) {
    // Active paid plan
    b.textContent = '💎 ' + plan;
    ...
    var daysLeft = (typeof ProTrialDaysLeft === 'function') ? ProTrialDaysLeft() : 0;
    ...
    // FIX 3c: Admin-granted trial from SP_PROFILE.trialExpiry.
    var adminDays = (typeof GetTrialDaysLeft === 'function') ? GetTrialDaysLeft() : 0;
    b.textContent = '⏳ Trial: ' + adminDays + 'd left';
    b.style.color = 'var(--accent)'; b.style.borderColor = 'rgba(0,200,150,0.3)';
  } else if (SP_PENDING_PAY) {
...
function RenderUpgrade() {
  ...
  const plan = (SP_PROFILE && SP_PROFILE.plan && SP_PROFILE.plan !== 'free') ? SP_PROFILE.plan : null;
  const expired = plan && SP_PROFILE.planExpiry && SP_PROFILE.planExpiry < today;
  if (plan && !expired) inner += '<div class="modal-sub">Current plan: <strong style="color:var(--accent)">' + escapeHtml(plan) + '</strong>' + (SP_PROFILE.planExpiry ? ' · valid till ' + SP_PROFILE.planExpiry : '') + '</div>';
  if (expired) inner += '<div class="modal-sub" style="color:var(--red)">Aapka ' + escapeHtml(plan) + ' plan expire ho gaya (' + SP_PROFILE.planExpiry + '). Renew karo ↓</div>';
  if (SP_PENDING_PAY) {
    inner += '<div style="background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.3);border-radius:8px;padding:10px 14px;font-size:0.82rem;color:var(--amber);margin-bottom:1rem;line-height:1.6;">⏳ Aapki payment (Txn: <strong>' + escapeHtml(SP_PENDING_PAY.txnId || '') + '</strong> · ₹' + (SP_PENDING_PAY.amount || 0) + ' · ' + escapeHtml(SP_PENDING_PAY.planName || '') + ') admin verification mein hai. Verify hote hi plan activate ho jayega.</div>';
  }
  if (!SP_PLANS.length) {
    ...
    SP_PLANS.forEach(function(p, idx) {
      ...
      inner +=
        ...
        '<button onclick="PickPlan(\'' + p.id + '\')" style="width:100%;padding:10px;border-radius:10px;border:none;background:' + (isPopular ? 'var(--accent)' : 'var(--border)') + ';color:' + (isPopular ? '#fff' : 'var(--text)') + ';font-weight:700;font-size:0.88rem;cursor:pointer;">Buy Now</button>' +
        '</div>';
    });
    inner += '</div>';
  }
  /* ── Free vs Pro comparison + self-serve 3-day trial ── */
  inner += FreeProCompareHtml();
...
function FreeProCompareHtml() {
  var rows = [
    ['App access', 'Full app — every page unlocked', 'Full app, no usage limits + advanced tools'],
    ['Exams', '1 (your target)', 'All exams + switching'],
    ['Mock tests — saves & analysis', 'Up to ' + SP_FREE_LIMITS.mocks + ' saves, no analysis', 'Unlimited saves + full analysis (trends, weak areas, percentile)'],
    ['Video notes & saved playlists', 'Up to ' + SP_FREE_LIMITS.notes + ' notes · ' + SP_FREE_LIMITS.mediaSaves + ' playlists/links', 'Unlimited notes · up to 10 playlists/links'],
  ...
  const _expiredPlan = SP_PROFILE && SP_PROFILE.plan && SP_PROFILE.plan !== 'free'
    && SP_PROFILE.planExpiry && SP_PROFILE.planExpiry < _todayForBadge;
  if (_expiredPlan) {
    ...
    trialBtn = '<div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:10px 14px;margin-top:12px;text-align:center;">' +
      '<div style="font-size:.82rem;color:#EF4444;font-weight:700;margin-bottom:6px;">⚠ Aapka ' + escapeHtml(SP_PROFILE.plan) + ' plan expire ho gaya (' + SP_PROFILE.planExpiry + ')</div>' +
      ...
      '</div>';
  } else if (currentUser && typeof IsPro === 'function' && !IsPro() && !ProTrialUsed()) {
    // No plan, no trial used yet — offer free trial
    trialBtn = '<button class="btn-modal-save" style="width:100%;margin-top:12px;" onclick="StartProTrial()">🎁 Start 3-day free Pro trial</button>' +
    ...
    trialBtn = '<div style="font-size:.78rem;color:var(--accent);text-align:center;margin-top:12px;font-weight:700;">⏳ Pro trial active — ' + ProTrialDaysLeft() + ' din baaki</div>';
  } else if (typeof ProTrialUsed === 'function' && ProTrialUsed() && !IsProTrialActive()) {
...
function PickPlan(pid) {
  _ezPickedPlan = SP_PLANS.find(p => p.id === pid); if (!_ezPickedPlan) return;
  _ezCoupon = null;
  _ezFinalAmount = _ezPickedPlan.price || 0;
  const upi = SP_UPI && SP_UPI.upiId ? SP_UPI : SP_UPI_FALLBACK;
  const amount = _ezFinalAmount;
  const upiDeep = 'upi://pay?pa=' + encodeURIComponent(upi.upiId) +
    '&pn=' + encodeURIComponent(upi.payeeName || 'StudyPlanner') +
    '&am=' + amount + '&cu=INR' +
    '&tn=' + encodeURIComponent('StudyPlanner ' + _ezPickedPlan.name);
  ...
  step.innerHTML =
      ...
      '<div style="flex:1;min-width:200px;">' +
        '<ol style="font-size:0.82rem;color:var(--muted);padding-left:1.2rem;line-height:2;margin-bottom:10px;">' +
          ...
          '<li>UPI ID: <strong style="color:var(--accent)">' + escapeHtml(upi.upiId) + '</strong>' +
            ' <button class="yt-dur-set-btn" onclick="CopyUpi(\'' + escapeHtml(upi.upiId) + '\')">Copy</button></li>' +
    ...
    '<div id="ez-coupon-row" style="display:flex;gap:6px;margin-bottom:12px;align-items:center;flex-wrap:wrap;background:var(--surface);padding:10px 12px;border-radius:10px;border:1px solid var(--border);">' +
      ...
      '<button class="yt-dur-set-btn" onclick="ApplyCoupon()">Apply</button>' +
      '<button id="ez-coupon-remove" class="yt-dur-set-btn" style="display:none;color:var(--red);" onclick="RemoveCoupon()">Remove</button>' +
    ...
    '<button class="btn-modal-save" style="width:100%;" onclick="SubmitPayment()">Submit for Verification</button>';
...
async function ApplyCoupon() {
  ...
  try {
    ...
    if (c.firstTimeOnly && currentUser && currentUser.uid) {
      ...
      const profile = u && u.p ? u.p : (SP_PROFILE || {});
    ...
    const upiDeep = 'upi://pay?pa=' + encodeURIComponent(upi.upiId) +
      ...
      '&tn=' + encodeURIComponent('StudyPlanner ' + _ezPickedPlan.name + ' [' + code + ']');
    const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=' + encodeURIComponent(upiDeep);
...
loginUser = function(email, name, uid, state) {
  ...
  setTimeout(function() { LoadPlans().then(function() { return loadProfile(); }); }, 400);
};
