/* ══════════════════════════════════════════════
   STUDYPLANNER PHASE 3 — REFERRAL SHARE WIDGET + PAYOUT PROGRESS
══════════════════════════════════════════════ */
function refLink() {
...
function renderRefWidget() {
  ...
  w.innerHTML = '<div class="info-card" style="margin-bottom:1.5rem;">' +
    ...
    '<button class="btn-modal-save" onclick="copyRef()">📋 Copy</button>' +
    '<button class="btn-modal-save" style="background:#25D366;color:#fff;" onclick="shareWa()">WhatsApp</button></div>' +
...
function shareWa() {
  const msg = '🎯 StudyPlanner — SSC/Railway/Banking/UPSC ki smart study planner! Syllabus tracker, mock test analysis, YouTube course organiser — sab ek jagah. Mere link se join karo: ' + refLink();
  window.open('https://wa.me/?text=' + encodeURIComponent(msg), '_blank');
}

/* Render widget after profile load and on dashboard refreshes */
const _ezLoadProfileBase3 = loadProfile;
