 ...
 */
const { buildTaskSections, todayTotalTasks, progressBar, fmtShort, escHtml } = require('./telegram-lib');
...
async function sendTelegramMessageWithKeyboard(botToken, chatId, text) {
  ...
  if (!data.ok) {
    ...
    if (data.error_code === 403 || (data.error_code === 400 && desc.includes('chat not found'))) throw Object.assign(new Error(desc), { skip: true });
...
function calculateStreak(appState) {
  ...
  let today = iso(d);
  const todayDone = (tasks[today] || []).some(t => t && (t.done === true || t.status === 'done'));
...
function weakArea(todoLines) {
  ...
  for (const t of todoLines) { const e = subjectEmoji(t.rawText || ''); if (e !== '•') counts.set(e, (counts.get(e) || 0) + 1); }
  let best = null;
...
function buildMorningDashboard(name, appState, topicDigest, dateStr) {
  ...
  if (priorities.length) {
    ...
    priorities.forEach((t, i) => { const raw = t.rawText || 'Task'; L.push(`${['①','②','③'][i] || `${i + 1}.`} ${subjectEmoji(raw)} ${escHtml(raw.length > 65 ? raw.slice(0, 65) + '…' : raw)}`); });
    L.push('');
  }
  if ((topicDigest && topicDigest.trim()) || pending.length || overdue.length) {
    L.push('<b>📚 STUDY PLAN</b>');
    if (topicDigest && topicDigest.trim()) topicDigest.trim().split('\n').filter(Boolean).slice(0, 4).forEach(line => L.push(`${subjectEmoji(line)} ${escHtml(line)}`));
    pending.slice(0, 6).forEach(t => { const raw = t.rawText || 'Task'; L.push(`○ ${subjectEmoji(raw)} ${escHtml(raw.length > 70 ? raw.slice(0, 70) + '…' : raw)}`); });
  ...
  if (weak) { L.push('<b>⚠️ NEEDS ATTENTION</b>'); L.push(`${weak.emoji} ${escHtml(weak.name.length > 65 ? weak.name.slice(0, 65) + '…' : weak.name)}  •  ${weak.count} pending`); L.push(''); }
  L.push('━━━━━━━━━━━━━━━━━━━━');
...
function buildEveningDashboard(name, appState, dateStr) {
  ...
  if (completed.length) {
    ...
    completed.slice(0, 6).forEach(t => { const raw = t.text || 'Task'; L.push(`✓ ${subjectEmoji(raw)} ${escHtml(raw.length > 70 ? raw.slice(0, 70) + '…' : raw)}`); });
  ...
  if (overdue.length) {
    ...
    overdue.slice(0, 4).forEach(t => { const raw = t.rawText || 'Task'; L.push(`↻ ${subjectEmoji(raw)} ${escHtml(raw.length > 70 ? raw.slice(0, 70) + '…' : raw)}`); });
    L.push('');
  }
  if (videoItems.length) {
    L.push('<b>🎬 VIDEOS</b>');
...
module.exports = { sendTelegramMessageWithKeyboard, morningKeyboard, eveningKeyboard, buildMorningDashboard, buildEveningDashboard };
