...
(async () => {
  ...
  assert.match(page, /id="youtubePlayer"/);
  assert.match(page, /Search through the RonFlix server/);
  assert.doesNotMatch(page, /ytSearchRonflixVideo|youtubeApiSetup|youtubeApiKey|data-yt-search-mode/);
  assert.match(search, /window\.RonflixStream\.request/);
  assert.match(search, /playYouTubeEmbed\(id,title,false\)/);
  assert.match(search, /youtube\.com\/embed/);
  assert.doesNotMatch(search, /youtubeApi|YOUTUBE_API|api\/info|api\/stream|Turbo|youtubeApiSetup/);
