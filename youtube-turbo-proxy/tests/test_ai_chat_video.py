...
check("OpenRouter Veo durations are normalized", "OPENROUTER_VIDEO_DURATION_OPTIONS" in SRC and "def _openrouter_video_duration" in SRC and 'return 6 if 6 in options' in SRC)
check("OpenRouter credits errors are actionable", "insufficient credits" in SRC and "Add credits or use a funded OpenRouter key" in SRC)
...
check("Unavailable OmniRoute route stops redundant model retries", "retrying eight more model IDs" in SRC and "route_unavailable = True" in SRC and "break" in SRC)
check("Frontend Video selector adapts duration to the chosen model", "function videoDurationOptions(modelKey)" in JS and "google/veo-3.1" in JS and "renderVideoDurationOptions" in JS)
