...
constants_end = SRC.index("def _omniroute_typed_catalog", constants_start)
constants = SRC[constants_start:constants_end]
ns = {}
exec(constants, ns)
order_start = SRC.index("def _omniroute_video_order", constants_start)
order_end = SRC.index("def _omniroute_typed_catalog", order_start)
...
check("direct video rejection falls back to the proxy", "return r.ok ? r : proxyRequest()" in JS and "catch(function () { return proxyRequest(); })" in JS)
